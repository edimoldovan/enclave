...
The spreadsheet, joining the suite as Enclave Grido. Grido's own roadmap stays in its repo; this is the suite
integration plan.

## Enclave suite (Grido)

Grido joins the Enclave suite as **Enclave Grido** (product name;
...
- [ ] Launchable from the Enclave tray + an "Enclave Grido" app
      search stub, alongside its normal standalone life.
- [ ] Naming pass: Grido branding when launched as part of the
      suite.
...
Grido is the one suite product whose logic is already Rust and already
...
`grido_*` tool definitions inside the one `enclave-mcp`;
routing each call to a Grido window; launching Grido when none runs;
...
socket** and holds the connection, announcing itself as a Grido
...
- `grido_overview` — every open workbook: path, sheets, used
  ranges, header rows, unsaved flag. The call that starts a task.
- `grido_read` — a window of values, or formulas, for a range;
  defaults to the used range and hard-caps the cell count.
- `grido_table` — header row as a schema plus the records
  matching a criteria object. How to say "the Acme row" reliably.
- `grido_stats` — sum/avg/count/min/max over a range, so
  questions are answered without dragging cells into context.
- `grido_find` — value search across sheets, hits as addresses.

Acting:

- `grido_open` — open an .xlsx in a Grido window: a path, or a
  copy handed over by Post or Depot.
- `grido_set` — cells, ranges and formulas.
- `grido_record` — append or update rows by header match.
- `grido_format` — styles and number formats on a range.
- `grido_structure` — rows, columns and sheets; add or remove.
- `grido_sort` — sort or filter a block.
- `grido_save` — write the file, or save a copy elsewhere.
- `grido_send` — hand the workbook to `@person` as a drop, or
  to a Post draft as an attachment.
- `grido_undo` — take back the last agent action.
...
Grido opens from the tray menu, from an "Enclave Grido" app-search
stub, from the .xlsx file association, or from the agent via
`grido_open`. All four hit one entry point with a suite flag:
title and mark become Enclave Grido, standalone socket and
...
- **Post → Grido.** "Open the attachment and fix it": Post saves the
  xlsx locally and calls `grido_open`. `grido_send`
  with a Post target puts the edited file back on the reply draft; the
  human confirms the send in Post.
- **Depot → Grido.** A colleague's share is read-only, so the agent
...
- **Grido → enclaved.** "Send the forecast to @ale" saves, then drops
  the file on @ale's online computer — queued if it is asleep. Same
  @-addressing everywhere, `@person/computer` when it matters.
- **Bursar → Grido.** Bursar exports a period's figures as a workbook
  and opens it here for the human to check before the report goes out.
  Bursar owns the numbers; Grido is where they are read.
- **Grido → Scribe / Podium.** A range travels into a document or a
  slide as a table: a snapshot of values and formats, never a live link.
- **Grido → Chat.** A range pastes into a channel as a small table;
...
recalculation, no headless Grido (the agent edits a window a human can
watch), no live cross-machine cell links, no second MCP registration,
and no transport code in Grido — sending, fetching and identity are
