//! Post's keymap: the file it loads and the commands the chords name.
//!
...
/// Loads the first keymap found in $POST_KEYMAP, ./keymap.toml,
/// ~/.config/post/keymap.toml, falling back to the embedded default.
/// Returns the keymap plus any warnings worth showing the user.
pub fn load() -> (Keymap, Vec<String>) {
    Keymap::load("post", DEFAULT_KEYMAP)
}
