...
//! email in a terminal. Three commands open it and each one *is* a view —
//! `enclave post` the accounts, `enclave post list <email>` that account's
//! inbox, `enclave post read <email> <id>` that message — so the window needs
//! no launcher and no menu to be useful: the command is the navigation.
//!
...
//!
//! - [`nav`] — the three views and the stack, including which command line
...
//! - [`ui`] — one `impl PostApp` block per area of the interface.
